...
export default function Calculator13({ key, result, callback }: any) {
    ...
    >
      ...
      >
        <div
          ...
          onClick={() => {
            setResult2("");
            purchaseOrderState[0].isTimeCalculator = false;
          }}
        >
        ...
        <div className="flex h-[60px] w-full items-center justify-end rounded-md bg-neutral-900 px-3 text-4xl text-white">
          {Number(result.setTime) != 0 && Number(result2) == 0
            ? Number(result.setTime).toLocaleString()
